...
fn test_trace_bitmask_bfs_generic_stride_16() {
    ...
    // Low node is 1. Dir = 1.
    // idx = 1 * 4 + 1 = 5 (power-of-4 encoding).
    // Word 0, bit 5.

    assert_ne!(
        decoder.edge_bitmap[0] & (1 << 5),
        0,
...
fn test_trace_bitmask_bfs_generic_3d() {
    ...
    // Low node b. Dir = 2 (Z).
    // idx = b * 4 + 2 (power-of-4 encoding).

    let idx = (b as usize) * 4 + 2;
    let word = idx / 64;
    let bit = idx % 64;
    ...
    assert_ne!(
        ...
        0,
        "Boundary correction should be marked"
    );
}
...
fn test_trace_bfs_explicit() {
    ...
    // 0-1: u=0, dir=0 (diff 1). idx=0*4+0=0.
    // 1-2: u=1, dir=0. idx=1*4+0=4.
